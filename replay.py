...
def replay_demonstration(
    episode_idx=0, zarr_path="./recordings/replay_buffer.zarr", debug=False
):
    """
    ...
    if episode_idx < 0 or episode_idx >= total_episodes:
        logger.error(
            f"Episode index {episode_idx} is out of range. Available episodes: 0-{total_episodes-1}"
        )
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Replay a demonstration from the XArm replay buffer"
    )
    parser.add_argument(
        "episode",
        type=int,
    ...
    parser.add_argument(
        "--path",
        type=str,
        default="./recordings/replay_buffer.zarr",
    ...
    replay_demonstration(
        episode_idx=args.episode, zarr_path=args.path, debug=args.debug
    )
