...
def check_modbus_baud(
    baud=2000000, _type="set", default=None, host_id=XCONF.TGPIO_HOST_ID
):
        ...
        def decorator(self, *args, **kwargs):
            ...
            if code != 0:
                ...
                return (
                    code
                    if _type == "set"
                    else (code, default if default != -99 else [])
                )
            else:
        ...
        def decorator(self, *args, **kwargs):
            if self.connected:
                ...
                return (
                    ...
                    if _type == "set"
                    else (APIState.NOT_CONNECTED, "xArm is not connect")
                )
        ...
        def decorator(self, *args, **kwargs):
            ...
            if self.connected:
                if self.check_xarm_is_ready:
                    ...
                    logger.info(
                        "Please check the arm for errors. If so, please clear the error first. "
                        "Then enable the motor, set the mode and set the state"
                    )
                    return (
                        APIState.NOT_READY
                        if _type == "set"
                        else (APIState.NOT_READY, "xArm is not ready")
                    )
            else:
                logger.error("xArm is not connected")
                return (
...
def api_log(func):
    ...
    def decorator(self, *args, **kwargs):
        ...
        logger.info(
            "{}, ret={}, args={}, kwargs={}".format(
                func.__name__, ret, args[1:], kwargs
            )
        )
        return ret
