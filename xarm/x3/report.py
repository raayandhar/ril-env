...
class ReportHandler(object):
    ...
    def process_report_data(self, recv_data):
        ...
        if len(self.buffer) < self.report_size:
            return
        if (
            self.report_type == "rich"
            and self.report_size == 233
            and len(self.buffer) >= 245
        ):
            ...
            try:
                if len(self.buffer) >= 249:
                    if convert.bytes_to_u32(self.buffer[245:249]) != self.report_size:
                        if (
                            convert.bytes_to_u32(self.buffer[233:237])
                            == self.report_size
                        ):
    ...
    def _parse_report_tcp_rich_data(self, rx_data):
        ...
        if length >= 201:
            ...
            ret.extend(
                [tcp_jerk, min_tcp_acc, max_tcp_acc, min_tcp_speed, max_tcp_speed]
            )
        if length >= 221:
