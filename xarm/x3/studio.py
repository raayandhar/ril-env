...
class Studio(object):
    ...
    def run_blockly_app(self, name, **kwargs):
        try:
            self.call_studio_api(
                {}, api_name="Core.command.xarm_set_blockly_init", show_fail_log=False
            )
        except:
            pass
        kwargs["appName"] = name
    ...
    def delete_blockly_app(self, name):
        ret = self.call_studio_api(
            {"parentPath": name, "selectNode": {"type": "file"}},
    ...
    def set_initial_point(self, point):
        ret = self.call_studio_api(
            {"point": point}, api_name="Core.command.xarm_set_initial_point"
        )
        if ret:
            return ret["code"]
        return APIState.API_EXCEPTION
    ...
    def __call_remote_api(self, *args, **kwargs):
        ...
        if self.__ip and api_name:
            r = self.__session.post(
                "http://{}:18333/{}".format(self.__ip, path),
                data=json.dumps({"cmd": api_name, "args": args, "kwargs": kwargs}),
                timeout=(5, None),
            )
            if r.status_code == 200:
                ...
                if show_fail_log:
                    logger.error(
                        "request failed, http_status_code={}".format(r.status_code)
                    )
        else:
            if show_fail_log:
                logger.error(
                    "ip or api_name is empty, ip={}, api_name={}".format(
                        self.__ip, api_name
                    )
                )
