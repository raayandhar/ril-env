...
class Servo(Base):
    ...
    def get_servo_debug_msg(self, show=False, lang="en"):
        ...
        if self._check_code(ret[0]) == 0:
            for i in range(1, 9):
                ...
                name = (
                    ("伺服-{}".format(i) if lang == "cn" else "Servo-{}".format(i))
                    if i < 8
                    else ("机械爪" if lang == "cn" else "Gripper")
                )
                dbmsg.append(
                    {
                        "name": name,
        ...
        if show:
            ...
            for servo_info in dbmsg:
                color = (
                    "red"
                    if servo_info["code"] != 0 or servo_info["status"] != 0
                    else "white"
                )
                pretty_print(
                    "* {}, {}: {}, {}: {}, {}: {}".format(
    ...
    def set_servo_zero(self, servo_id=None):
        ...
        self.log_api_info(
            "API -> set_servo_zero(servo_id={}) -> code={}".format(servo_id, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_servo_addr_16(self, servo_id=None, addr=None, value=None, id_check=True):
        ...
        if id_check:
            assert (
                isinstance(servo_id, int) and 1 <= servo_id <= 7
            ), "The value of parameter servo_id can only be 1-7."
        ...
        self.log_api_info(
            "API -> set_servo_addr_16(servo_id={}, addr={}, value={}) -> code={}".format(
                servo_id, addr, value, ret[0]
            ),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_servo_addr_32(self, servo_id=None, addr=None, value=None, id_check=True):
        ...
        self.log_api_info(
            "API -> set_servo_addr_32(servo_id={}, addr={}, value={}) -> code={}".format(
                servo_id, addr, value, ret[0]
            ),
            code=ret[0],
        )
    ...
    def get_servo_error_code(self, servo_id=None):
        assert servo_id is None or (
            isinstance(servo_id, int) and servo_id >= 1
            ...
            for i in range(count):
                ...
                if ret[0] == XCONF.UxbusState.ERR_CODE:
                    ...
                    if _ == 0:
                        ...
                        code = _
                        logger.error(
                            "Get controller errwarn: ret={}, errwarn={}".format(
                                code, err_warn
                            )
                        )
                        errcodes[i] = ret[1]
    ...
    def get_servo_all_pids(self, servo_id=None):
        ...
        self.clean_warn()
        addrs = [
            XCONF.ServoConf.POS_KP,
            XCONF.ServoConf.POS_FWDKP,
            XCONF.ServoConf.POS_PWDTC,
            XCONF.ServoConf.SPD_KP,
            XCONF.ServoConf.SPD_KI,
            XCONF.ServoConf.CURR_KP,
            XCONF.ServoConf.CURR_KI,
            XCONF.ServoConf.SPD_IFILT,
            XCONF.ServoConf.SPD_OFILT,
            XCONF.ServoConf.CURR_IFILT,
            XCONF.ServoConf.POS_KD,
            XCONF.ServoConf.POS_CMDILT,
            XCONF.ServoConf.GET_TEMP,
            XCONF.ServoConf.OVER_TEMP,
        ]
        if servo_id is None or servo_id > self.axis:
            count = 7 if servo_id == 8 else self.axis
