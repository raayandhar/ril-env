...
class BaseBoard(Base):
    ...
    def write_sn(self, sn="", servo_id=10):
        ...
        if len(sn) == 14:
            for i in range(0, 14, 2):
                ret = self.arm_cmd.tgpio_addr_w16(
                    addr=0x1900 + (int(i / 2)),
                ...
                if code != 0:
                    self.log_api_info(
                        "API -> write_sn -> code={}, sn={}".format(code, sn), code=code
                    )
                    return code
        self.log_api_info(
            "API -> write_sn -> code={}, sn={}".format(code, sn), code=code
    ...
    def get_sn(self, servo_id=10):
        ...
        for i in range(0, 14, 2):
            ...
            if ret[0] != 0:
                self.log_api_info(
                    "API -> get_sn -> code={}, sn={}".format(ret[0], rd_sn), code=ret[0]
                )
                return ret[0], ""
        self.log_api_info(
            "API -> get_sn -> code={}, sn={}".format(ret[0], rd_sn), code=ret[0]
        ...
        if idens:
            for i, data in enumerate(idens):
                ...
                for j, d in enumerate(data):
                    ret = self.arm_cmd.tgpio_addr_w32(
                        addr=(cmds[i] + (2 * j)) | 0x1000, value=d, bid=servo_id
                    )
    ...
    def write_poe_to_end(self, datas, servo_id=9):
        ...
        if datas:
            ...
            for i, data in enumerate(joints):
                for j, d in enumerate(data):
                    ret = self.arm_cmd.tgpio_addr_w32(
                        addr=(cmds1[i] + (2 * j)) | 0x1000, value=d, bid=servo_id
                    )
                    time.sleep(0.1)
                    code = ret[0]
                    if code != 0:
            ...
            for i, data in enumerate(homes):
                for j, d in enumerate(data):
                    ret = self.arm_cmd.tgpio_addr_w32(
                        addr=(cmds2 + (2 * (i * len(data) + j))) | 0x1000,
                        value=d,
                        bid=servo_id,
                    )
                    time.sleep(0.1)
