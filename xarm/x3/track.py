...
class Track(GPIO):
    ...
    def _get_linear_track_registers(self, addr, number_of_registers=1):
        code = self.checkset_modbus_baud(
            self._default_linear_track_baud, host_id=XCONF.LINEER_TRACK_HOST_ID
        )
        if code != 0:
            return code, []
        ret = self.arm_cmd.track_modbus_r16s(addr, number_of_registers)
        ret[0] = self._check_modbus_code(
            ret, length=5 + 2 * number_of_registers, host_id=XCONF.LINEER_TRACK_HOST_ID
        )
        return ret[0], ret[1:]
    ...
    def get_linear_track_registers(self, addr=0x0A20, number_of_registers=8):
        assert (addr == 0x0A20 and number_of_registers >= 2) or (
            0x0A22 <= addr <= 0x0A27 and number_of_registers >= 1
        ), (
            "parameters error, only support (addr == 0x0A20 and number_of_registers >= 2) "
            "or (0x0A22 <= addr <= 0x0A27 and number_of_registers >= 1)"
        )
        code, data = self._get_linear_track_registers(
            addr, number_of_registers=number_of_registers
        )
        if code == 0:
            if addr == 0x0A20 and number_of_registers >= 2:
                self._linear_track_status["pos"] = round(
                    convert.bytes_to_long_big(data[4:8]) / 2000
                )
            if 0x0A22 - number_of_registers < addr <= 0x0A22:
                start_inx = (0x0A22 - addr) * 2 + 4
                self._linear_track_status["status"] = convert.bytes_to_u16(
                    data[start_inx : start_inx + 2]
                )
            if 0x0A23 - number_of_registers < addr <= 0x0A23:
                start_inx = (0x0A23 - addr) * 2 + 4
                self._linear_track_status["error"] = convert.bytes_to_u16(
                    data[start_inx : start_inx + 2]
                )
            if 0x0A24 - number_of_registers < addr <= 0x0A24:
                start_inx = (0x0A24 - addr) * 2 + 4
                self._linear_track_status["is_enabled"] = (
                    convert.bytes_to_u16(data[start_inx : start_inx + 2]) & 0x01
                )
                self.linear_track_is_enabled = (
                    self._linear_track_status["is_enabled"] == 1
                )
            if 0x0A25 - number_of_registers < addr <= 0x0A25:
                start_inx = (0x0A25 - addr) * 2 + 4
                self._linear_track_status["on_zero"] = (
                    convert.bytes_to_u16(data[start_inx : start_inx + 2]) & 0x01
                )
            if 0x0A26 - number_of_registers < addr <= 0x0A26:
                start_inx = (0x0A26 - addr) * 2 + 4
                self._linear_track_status["sci"] = (
                    convert.bytes_to_u16(data[start_inx : start_inx + 2]) >> 1 & 0x01
                )
    ...
    def set_linear_track_enable(self, enable):
        ...
        ret[0] = self._check_modbus_code(
            ret, length=8, host_id=XCONF.LINEER_TRACK_HOST_ID
        )
        # get_status: error, is_enable, on_zero
        code2, status = self.get_linear_track_registers(
            addr=0x0A23, number_of_registers=3
        )
        ...
        self.log_api_info(
            "API -> set_linear_track_enable(enable={}) -> code1={}, code2={}, err={}, enabled={}, zero={}".format(
                enable,
                ret[0],
                code2,
                status["error"],
        ...
        return (
            ret[0]
            if self.linear_track_error_code == 0
            else APIState.LINEAR_TRACK_HAS_FAULT
        )
    ...
    def set_linear_track_back_origin(self, wait=True, **kwargs):
        ...
        self.log_api_info(
            "API -> set_linear_track_back_origin() -> code1={}, code2={}, err={}, enabled={}, zero={}".format(
                ret[0], code2, status["error"], status["is_enabled"], status["on_zero"]
            ),
            code=ret[0],
        )
        if ret[0] == 0 and wait:
    ...
    def set_linear_track_pos(self, pos, speed=None, wait=True, timeout=100, **kwargs):
        ...
        auto_enable = kwargs.get("auto_enable", True)
        # get_status: error, is_enable, on_zero
        code, status = self.get_linear_track_registers(
            addr=0x0A23, number_of_registers=3
        )
        if code == 0 and status["on_zero"] != 1:
            logger.warn(
                "linear track is not on zero, please set linear track back to origin"
            )
        ...
        self.log_api_info(
            "API -> set_linear_track_pos(pos={}) -> code={}, err={}, enabled={}, zero={}".format(
                pos,
                ret[0],
                self._linear_track_status["error"],
                self._linear_track_status["is_enabled"],
                self._linear_track_status["on_zero"],
            ),
            code=ret[0],
    ...
    def set_linear_track_speed(self, speed):
        ...
        self.log_api_info(
            "API -> set_linear_track_speed(speed={}) -> code={}".format(speed, ret[0]),
            code=ret[0],
        )
        return ret[0]
    ...
    def set_linear_track_stop(self):
        ...
        # get_status: error, is_enable, on_zero
        code2, status = self.get_linear_track_registers(
            addr=0x0A22, number_of_registers=2
        )
        self.log_api_info(
            "API -> set_linear_track_stop() -> code={}, code2={}, status={}, err={}".format(
                ret[0], code2, status["status"], status["error"]
            ),
            code=ret[0],
        )
        return ret[0]
    ...
    def clean_linear_track_error(self):
        ...
        self.log_api_info(
            "API -> clean_linear_track_error -> code={}, code2={}, err={}".format(
                ret[0], _, err
            ),
            code=ret[0],
        )
    ...
    def __wait_linear_track_stop(self, timeout=100):
        ...
        while self.connected and time.monotonic() < expired:
            _, status = self.get_linear_track_registers(
                addr=0x0A22, number_of_registers=5
            )
    ...
    def get_linear_track_version(self):
        ...
        ret1[0] = self._check_modbus_code(
            ret1, length=7, host_id=XCONF.LINEER_TRACK_HOST_ID
        )
        ret2[0] = self._check_modbus_code(
            ret2, length=7, host_id=XCONF.LINEER_TRACK_HOST_ID
        )
        ret3[0] = self._check_modbus_code(
            ret3, length=7, host_id=XCONF.LINEER_TRACK_HOST_ID
        )
    ...
    def get_linear_track_sn(self):
        ...
        for i in range(0, 14):
            ...
            ret[0] = self._check_modbus_code(
                ret, length=7, host_id=XCONF.LINEER_TRACK_HOST_ID
            )
    ...
    def set_linear_track_default_parmas(self):
        ...
        is_on_zero = convert.u16_to_bytes(
            0
        )  # 回零完成状态默认值  0 --- 没有回零完成   1 --- 回零完成
