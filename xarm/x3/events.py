...
class Events(object):
    ...
    def _register_report_callback(self, report_id, callback):
        ...
        if (
            callable(callback) or isinstance(callback, dict)
        ) and callback not in self._report_callbacks[report_id]:
    ...
    def register_report_callback(
        ...
        return self._register_report_callback(
            REPORT_ID,
            {
                "callback": callback,
    ...
    def register_report_location_callback(
        self, callback=None, report_cartesian=True, report_joints=False
    ):
        ret = self._register_report_callback(
            REPORT_LOCATION_ID,
            {
                "callback": callback,
                "cartesian": report_cartesian,
    ...
    def register_mtable_mtbrake_changed_callback(self, callback=None):
        return self._register_report_callback(
            REPORT_MTABLE_MTBRAKE_CHANGED_ID, callback
        )
