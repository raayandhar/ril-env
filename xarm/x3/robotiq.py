...
class RobotIQ(Base):
    ...
    def __robotiq_get(self, params):
        ...
        if code == 0 and len(ret) >= 5:
            ...
            if self._robotiq_status["gSTA"] == 3 and (
                self._robotiq_status["gFLT"] == 0 or self._robotiq_status["gFLT"] == 9
            ):
    ...
    def robotiq_reset(self):
        ...
        self.log_api_info(
            "API -> robotiq_reset -> code={}, response={}".format(code, ret), code=code
        )
        return code, ret
    ...
    def robotiq_set_activate(self, wait=True, timeout=3):
        ...
        self.log_api_info(
            "API -> robotiq_set_activate ->code={}, response={}".format(code, ret),
            code=code,
        )
        if code == 0:
            self.robotiq_is_activated = True
    ...
    def robotiq_set_position(
        ...
        self.log_api_info(
            "API -> robotiq_set_position ->code={}, response={}".format(code, ret),
            code=code,
        )
        return code, ret

    def robotiq_open(self, speed=0xFF, force=0xFF, wait=True, timeout=5, **kwargs):
        return self.robotiq_set_position(
            0x00, speed=speed, force=force, wait=wait, timeout=timeout, **kwargs
        )

    def robotiq_close(self, speed=0xFF, force=0xFF, wait=True, timeout=5, **kwargs):
        return self.robotiq_set_position(
            0xFF, speed=speed, force=force, wait=wait, timeout=timeout, **kwargs
        )
    ...
    def robotiq_get_status(self, number_of_registers=3):
        number_of_registers = (
            3 if number_of_registers not in [1, 2, 3] else number_of_registers
        )
    ...
    def robotiq_wait_activation_completed(self, timeout=3):
        failed_cnt = 0
        expired = (
            time.monotonic() + timeout if timeout is not None and timeout > 0 else 0
        )
        ...
        while expired == 0 or time.monotonic() < expired:
            ...
            if _ == 0:
                ...
                code = (
                    ...
                    if gFLT != 0 and not (gFLT == 5 and gSTA == 1)
            ...
            if _ == 0:
                ...
                code = (
                    ...
                    if gFLT != 0 and not (gFLT == 5 and gSTA == 1)
                        ...
                        0
                        if (check_detected and (gOBJ == 1 or gOBJ == 2))
                        or (gOBJ == 1 or gOBJ == 2 or gOBJ == 3)
                        else code
                    )
                )
            else:
    ...
    def check_robotiq_is_catch(self, timeout=5):
        return (
            self.robotiq_wait_motion_completed(timeout=timeout, check_detected=True)
            == 0
        )
    ...
    def robotiq_calibrate_mm(self, closemm, openmm):
        ...
        if ret[0] == 0:
            ...
            if ret[0] == 0:
                ...
                self.__robotiq_bCoef = (openmm * close_bit - closemm * open_bit) / (
                    close_bit - open_bit
                )
    ...
    def robotiq_set_position_mm(
        ...
        if self.__robotiq_openmm is None or self.__robotiq_closemm is None:
            print(
                "You have to calibrate the gripper before using the function robotiq_set_position_mm()"
            )
    ...
    def __robotiq_mm_to_bit(self, mm):
        if self.__robotiq_aCoef is None or self.__robotiq_bCoef is None:
            print(
                "You have to calibrate the gripper before using the function robotiq_mm_to_bit()"
            )
        else:
    ...
    def __robotiq_bit_to_mm(self, bit):
        if self.__robotiq_aCoef is None or self.__robotiq_bCoef is None:
            print(
                "You have to calibrate the gripper before using the function robotiq_bit_to_mm()"
            )
        else:
