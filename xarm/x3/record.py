...
class Record(Base):
    ...
    def get_trajectories(self, ip=None):
        ...
        try:
            ...
            if res.code == 200:
                ...
                return result["res"][0], [
                    {"name": item["name"], "duration": item["count"] / 100}
                    for item in result["res"][1]
                ]
            else:
    ...
    def start_record_trajectory(self):
        ...
        self.log_api_info(
            "API -> start_record_trajectory -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def stop_record_trajectory(self, filename=None, **kwargs):
        ...
        if isinstance(filename, str) and filename.strip():
            ret2 = self.save_record_trajectory(
                filename, wait=True, timeout=10, **kwargs
            )
            if ret2 != 0:
                return ret2
        self.log_api_info(
            "API -> stop_record_trajectory -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def save_record_trajectory(self, filename, wait=True, timeout=2, **kwargs):
        ...
        ret = self.arm_cmd.save_traj(
            full_filename, wait_time=0, feedback_key=feedback_key
        )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        self.log_api_info(
            "API -> save_record_trajectory -> code={}".format(ret[0]), code=ret[0]
        )
    ...
    def __wait_traj_op(self, timeout, trans_id, filename="unknown", op="Load"):
        if self._support_feedback and trans_id > 0:
            ...
            if code == 0:
                success_status = (
                    XCONF.TrajState.LOAD_SUCCESS
                    if op == "Load"
                    else XCONF.TrajState.SAVE_SUCCESS
                )
                failure_status = (
                    XCONF.TrajState.LOAD_FAIL
                    if op == "Load"
                    else XCONF.TrajState.SAVE_FAIL
                )
                status = (
                    success_status
                    if feedback_code == XCONF.FeedbackCode.SUCCESS
                    else (
                        failure_status
    ...
    def load_trajectory(self, filename, wait=True, timeout=None, **kwargs):
        ...
        self.get_trajectory_rw_status()
        feedback_key, studio_wait = self._gen_feedback_key(wait, **kwargs)
        ret = self.arm_cmd.load_traj(
            full_filename, wait_time=0, feedback_key=feedback_key
        )
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        self.log_api_info(
            "API -> load_trajectory -> code={}".format(ret[0]), code=ret[0]
        )
        if ret[0] == 0 and wait:
    ...
    def playback_trajectory(
        self, times=1, filename=None, wait=False, double_speed=1, **kwargs
    ):
        ...
        if self.version_is_ge(1, 2, 11):
            ret = self.arm_cmd.playback_traj(
                times, double_speed, feedback_key=feedback_key
            )
        else:
            ret = self.arm_cmd.playback_traj_old(times)
        trans_id = self._get_feedback_transid(feedback_key, studio_wait)
        self.log_api_info(
            "API -> playback_trajectory -> code={}".format(ret[0]), code=ret[0]
        )
        if ret[0] == 0 and wait:
