...
class UxbusCmdTcp(UxbusCmd):
    def __init__(self, arm_port, set_feedback_key_tranid=None):
        super(UxbusCmdTcp, self).__init__(
            set_feedback_key_tranid=set_feedback_key_tranid
        )
    ...
    def __standard_modbus_tcp_request(self, pdu, unit_id=0x01):
        ret = self.send_modbus_request(
            unit_id, pdu, len(pdu), prot_id=STANDARD_MODBUS_TCP_PROTOCOL
        )
        if ret == -1:
            return XCONF.UxbusState.ERR_NOTTCP, b""
        ret = self.recv_modbus_response(
            unit_id, ret, -1, 10000, t_prot_id=STANDARD_MODBUS_TCP_PROTOCOL
        )
    ...
    def __read_registers(self, addr, quantity, func_code=0x03, is_signed=False):
        ...
        if code == 0 and len(res_data) == 9 + quantity * 2:
            return 0, list(
                struct.unpack(
                    ">{}{}".format(quantity, "h" if is_signed else "H"), res_data[9:]
                )
            )
        else:
            return code, res_data
    ...
    def read_holding_registers(self, addr, quantity, is_signed=False):
        ...
        return self.__read_registers(
            addr, quantity, func_code=0x03, is_signed=is_signed
        )
    ...
    def read_input_registers(self, addr, quantity, is_signed=False):
        ...
        """
        return self.__read_registers(
            addr, quantity, func_code=0x04, is_signed=is_signed
        )
    ...
    def write_and_read_holding_registers(
        ...
        pdu = struct.pack(
            ">BHHHHB", 0x17, r_addr, r_quantity, w_addr, len(w_regs), len(w_regs) * 2
        )
        pdu += convert.u16s_to_bytes(w_regs, len(w_regs))
        code, res_data = self.__standard_modbus_tcp_request(pdu)
        if code == 0 and len(res_data) == 9 + r_quantity * 2:
            return 0, struct.unpack(
                ">{}{}".format(r_quantity, "h" if is_signed else "H"), res_data[9:]
            )
        else:
            return code, res_data
