...
class _BlocklyBase(_BlocklyNode):
    ...
    def _get_condition_expression(self, value_block, arg_map=None):
        ...
        if block.attrib["type"] == "logic_boolean":
            ...
            return (
                '[" ".join([hex(da).split("0x")[1].upper().zfill(2) for da in data]) if isinstance(data, list) else data for '
                "data in self._arm.getset_tgpio_modbus_data({}, host_id={})][-1]".format(
                    int_li, host_id
                )
            )
            ...
            return (
                "[self._arm.ft_sensor_enable(1), self._arm.set_state(0), self._arm.get_ft_sensor_data()[1][{}]][2] if"
                " self._arm.get_ft_sensor_config()[1][1] == 0 else self._arm.get_ft_sensor_data()[1][{}]".format(
                    direction_li.index(direction), direction_li.index(direction)
                )
            )
            ...
            return (
                "list(map(lambda x: hex(x).split('0x')[1].upper().zfill(4)[:2] + ' ' + hex(x).split('0x')[1]."
                "upper().zfill(4)[2:], self._arm.read_holding_registers({}, 1)[1]))[0]".format(
                    str(addr)
                )
            )
