...
class _BlocklyHandler(_BlocklyBase):
    ...
    def _handle_move_axis_angle(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "if not self._check_code(code, 'set_position_aa'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_tool_message(self, block, indent=0, arg_map=None):
        ...
        msg = json.dumps(
            fields[2].text if fields[-1].text is not None else "", ensure_ascii=False
        )
        if self._highlight_callback is not None:
    ...
    def _handle_wait(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "for i in range({}):".format(int(value / 0.1)), indent + 2
        )
    ...
    def _handle_set_robotiq_init(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code("    return", indent + 2)
        self._append_main_code(
            "code, ret = self._arm.robotiq_set_activate(wait=True)", indent + 2
        )
        self._append_main_code(
    ...
    def _handle_set_robotiq_gripper(self, block, indent=0, arg_map=None):
        ...
        wait = fields[0].text == "TRUE" if fields and len(fields) > 0 else False
        self._append_main_code(
            "code, ret = self._arm.robotiq_set_position({}, speed={}, force={}, wait={})".format(
    ...
    def _handle_event_get_holding_condition(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "# Define Holding Registers {} Value is {} callback".format(addr, trigger),
            indent=1,
        )
    ...
    def __handle_count_event(self, count_type, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "# Define Counter Value is {} {} callback".format(
                "greater than" if op == ">" else "less than" if op == "<" else "",
                trigger,
            ),
            indent=1,
        )
    ...
    def _handle_python_code(self, block, indent=0, arg_map=None, **kwargs):
        ...
        for i, code in enumerate(codes):
            if not code.strip():
                ...
                prev_is_empty = False
            if (
                (self._is_exec or (not self._is_exec and not self._is_ide))
                and code.strip()
                and code not in ["finally:", "else:"]
                and all([i not in code for i in ["elif", "except", "def", "class"]])
    ...
    def _handle_set_end_level(self, block, indent=0, arg_map=None, **kwargs):
        ...
        if self.axis_type[0] == 5:
            ...
            self._append_main_code(
                "code = self._arm.set_servo_angle(angle=[*current_angle[:3], angle, current_angle[4]])",
                indent + 2,
            )
    ...
    def _handle_set_modbus_rtu(self, block, indent=0, arg_map=None):
        ...
        if not is_run_cmd:
            self._append_main_code(
                "if not self._check_code(-1, 'set_tgpio_modbus'):", indent + 2
            )
            self._append_main_code("    return", indent + 2)
            return
        ...
        self._append_main_code(
            "code, ret = self._arm.getset_tgpio_modbus_data({}, host_id={})".format(
                int_li, host_id
            ),
            indent + 2,
        )
        self._append_main_code(
            "if not self._check_code(code, 'set_tgpio_modbus'):", indent + 2
        )
        self._append_main_code("    return", indent + 2)
    ...
    def _handle_set_ft_sensor(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "code = self._arm.config_force_control({}, {}, {}, [0] * 6)".format(
                ref_frame, force_axis_value, force_ref_value
            ),
            indent + 2,
        )
        self._append_main_code(
        ...
        self._append_main_code(
            "while time.time() - start_time < {}:".format(wait_time), indent + 2
        )
    ...
    def _handle_studio_run_blockly(self, block, indent=0, arg_map=None):
        ...
        if not self._is_exec:
            self._append_main_code(
                'self._start_run_blockly(fileName="{}", times={})'.format(
                    fileName.lstrip("/"), times
                ),
                indent + 2,
            )
        else:
            self._append_main_code(
                'start_run_blockly(fileName="{}", times={})'.format(fileName, times),
                indent + 2,
            )
    ...
    def _handle_studio_run_gcode(self, block, indent=0, arg_map=None):
        ...
        if self._is_exec:
            self._append_main_code(
                'start_run_gcode(projectName="{}", fileName="{}", times={})'.format(
                    projectName, fileName, times
                ),
                indent + 2,
            )
        else:
            ...
            self._append_main_code(
                '    self._arm.run_gcode_app(path="{}")'.format(projectName + fileName),
                indent + 2,
            )
    ...
    def _handle_write_single_holding_register(self, block, indent=0, arg_map=None):
        ...
        self._append_main_code(
            "self._arm.write_single_holding_register({}, {})".format(addr, value),
            indent + 2,
        )
